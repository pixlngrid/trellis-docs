...
4. `globals.css` maps these tokens to semantic CSS variables (for example, `--primary`, `--muted`).
...
components/docs/
├── mdx/
│   ├── index.tsx            # MDX (Markdown with JSX) component registry
...
│   ├── image-lightbox.tsx   # Select-to-zoom image modal
...
| `heading.tsx` | Heading anchors with select-to-copy URL functionality |
...
| `image-lightbox.tsx` | Client-side | Select-to-zoom modal for images in MDX content |
