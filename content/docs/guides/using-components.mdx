---
title: Using Components
description: Learn about the interactive React components available in the framework.
keywords: [components, react, interactive]
last_update:
  date: 02/20/2026
  author: Trellis
---

The Trellis framework includes several reusable React components you can use in your MDX files.

## Design tokens

The framework uses a design token system that converts a JSON file (`design-tokens.json`) into CSS custom properties at build time.

```json title="design-tokens.json (excerpt)"
{
  "brand": {
    "primary-600": {
      "value": "#2a5a82",
      "type": "color"
    }
  },
  "accent": {
    "olive-400": {
      "value": "#b4ca4a",
      "type": "color"
    }
  }
}
```

These become CSS variables like `--color-brand-primary-600` and `--color-accent-olive-400` that you can reference throughout your styles.

## Theme customizations

### Last updated at top

The last-updated date and edit links appear **at the top** of each page, right below the title. This gives readers immediate context about content freshness.

### Heading anchors

Hover over any heading to see a link icon. Select it to copy the anchor URL to your clipboard.

### Tab URL sync

When you switch tabs, the framework syncs the selection to the URL query parameters. You can then share a link that opens on the correct tab.

## Custom admonition icons

The framework includes custom SVG icons for each admonition type (note, tip, info, caution, danger) that provide a cleaner visual experience than the defaults.

## Plugins

### Smart search

The search plugin indexes all documentation at build time and provides a fast, client-side search experience. Configure weights in `config/site.ts`:

```js
{
  search: {
    weights: {
      title: 1.0,
      'sections.heading': 1.0,
      keywords: 0.8,
      description: 0.6,
      'sections.content': 0.5,
      content: 0.4,
    }
  }
}
```

### Redirects

Manage URL redirects via `redirects.json`:

```json
[
  { "from": "/old-page/", "to": "/new-page/" }
]
```

### Image lightbox

All images in markdown content automatically support zoom via the lightbox plugin. No additional markup needed.

## Tooltip

Add hover tooltips to define terms inline. The `Tooltip` component is globally registered — no import needed.

```mdx
The team agreed on new <Tooltip content="A target or goal for the performance or quality of a service.">SLOs</Tooltip> for the quarter.
```

See the [Components Overview](/components/) for full props and usage.

## Creating custom components

You can add your own React components and make them available in MDX — either via explicit `import` or as **global components** that are available on every page without importing.

### 1. Create the component

Add a new file under `components/custom/`:

```tsx title="components/custom/status-badge.tsx"
'use client'

interface StatusBadgeProps {
  status: 'stable' | 'beta' | 'deprecated'
}

const colors = {
  stable: 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200',
  beta: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200',
  deprecated: 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200',
}

export function StatusBadge({ status }: StatusBadgeProps) {
  return (
    <span className={`inline-block px-2 py-0.5 text-xs font-medium rounded ${colors[status]}`}>
      {status}
    </span>
  )
}
```

:::tip
Add `'use client'` at the top if the component uses hooks, event handlers, or browser APIs. Pure display components that only accept props can remain server components.
:::

### 2. Use via import (per-page)

Import it directly in any MDX file:

```mdx
import { StatusBadge } from '@/components/custom/status-badge';

This feature is <StatusBadge status="beta" />.
```

This works immediately — no registration needed.

### 3. Register as a global component (optional)

To make a component available on **every page** without importing, add it to `components/docs/mdx/index.tsx`:

```ts title="components/docs/mdx/index.tsx"
import { StatusBadge } from '@/components/custom/status-badge'

export const mdxComponents: Record<string, React.ComponentType<any>> = {
  // ... existing components
  StatusBadge,
}
```

You can then use it in any MDX file without an import:

```mdx
This feature is <StatusBadge status="stable" />.
```

### Server vs. client components

| Component type | When to use | Directive |
|----------------|-------------|-----------|
| **Server** (default) | Static display, no interactivity | None needed |
| **Client** | Hooks, state, event handlers, browser APIs | `'use client'` at top of file |

All globally registered components in `mdxComponents` are rendered inside a server component (`MDXRemote`), but client components work fine — React handles the boundary automatically.

### File organization

```
components/
├── custom/              ← Your custom components
│   ├── glossary.tsx
│   ├── feedback.tsx
│   ├── status-badge.tsx ← New component
│   └── ...
└── docs/
    └── mdx/
        └── index.tsx    ← Global registration
```