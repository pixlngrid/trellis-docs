...
The title bar appears above the code block with a muted background. Code blocks without a `title` render normally with no extra bar.
...
- **Use `title` for filenames** — When showing config files or code that readers will create, add `title="path/to/file"` so they know where to put it
- **Use meta line numbers for multiple highlights** — `{2,4-5}` is cleaner than adding `[!code highlight]` to every line
- **Use inline comments for single-line emphasis** — `[!code highlight]` is better when you want to draw attention to one specific line in a longer block
- **Use diff notation for before/after** — `[!code ++]` and `[!code --]` are clearer than describing changes in prose
- **Use focus for teaching** — `[!code focus]` dims everything except the important lines, which is ideal for tutorials where you want to draw attention to a specific part of a larger snippet
