...
| Setting | Description |
|---------|-------------|
| `logo.navbar` | Image path for the navbar logo (relative to `public/`). Set to `null` to hide. |
| `logo.hero` | Image path for the landing page hero. Set to `null` to hide. |
...
Add your GA4 measurement ID to enable Google Analytics tracking across your site. The gtag script loads with `afterInteractive` strategy so it never blocks page rendering.

| Setting | Description |
|---------|-------------|
| `analytics.enabled` | Set to `true` to load the Google Analytics script |
| `analytics.googleAnalyticsId` | Your GA4 measurement ID (e.g., `G-XXXXXXXXXX`) |
...
When `enabled` is `false` or `googleAnalyticsId` is empty, no tracking script is loaded.

### SEO

{vars.productName} generates Open Graph, Twitter Card, and JSON-LD structured data for every page automatically. Configure the defaults here — per-page metadata comes from frontmatter.

| Setting | Description |
|---------|-------------|
| `seo.ogImage` | Default Open Graph image path (1200x630px recommended). Used when a page has no cover image. |
| `seo.twitterHandle` | Your Twitter/X handle (e.g., `@yourhandle`). Adds `twitter:creator` meta tag. |
| `seo.twitterCardType` | `'summary'` for a small image card, or `'summary_large_image'` for a full-width image card |

```ts title="config/site.ts"
seo: {
  ogImage: '/img/og-default.png',
  twitterHandle: '@acmedocs',
...
- **Sitemap** — `public/sitemap.xml` is generated at build time with all doc, blog, and release note URLs. Includes `lastmod`, `changefreq`, and `priority` values.
- **robots.txt** — `public/robots.txt` is generated at build time pointing to your sitemap.
- **Canonical URLs** — every page gets a `<link rel="canonical">` tag.
- **Open Graph tags** — title, description, URL, type, and image for every page.
- **Twitter Cards** — card type, title, description, creator, and image.
- **JSON-LD structured data** — `WebSite` schema on the root layout, `Article` schema on every doc page (includes headline, description, URL, and dateModified).
- **Keywords** — the `keywords` frontmatter array is now rendered as a `<meta name="keywords">` tag in addition to being used for search indexing.

See [Head Metadata](/guides/markdown/head-metadata/) for per-page metadata details.
