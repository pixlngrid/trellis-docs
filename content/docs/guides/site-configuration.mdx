...
Choose between two blog designs. Change the `layout` value to switch instantly — no other changes needed.
...
Choose between two release notes designs. The content sections (`## Added`, `## Changed`, `## Fixed`, `## Removed`) are automatically detected and styled.
...
### Subscribe & Feedback CTAs

Optional call-to-action blocks that appear on blog and release notes pages. Both fields accept a URL or an email address (auto-detected and converted to `mailto:`).

| Setting | Description |
|---------|-------------|
| `subscribeUrl` | External subscribe link (e.g., Mailchimp, Buttondown, or an email address). Shown on blog and release notes landing pages when set. |
| `feedbackUrl` | External feedback link (e.g., Google Form, Typeform, or an email address). Shown on release note articles when set. |

```ts title="config/site.ts"
subscribeUrl: 'https://buttondown.com/your-newsletter',
feedbackUrl: 'https://forms.gle/your-form-id',
```

The subscribe CTA renders a dark gradient card with a "Subscribe" button. The feedback CTA renders "Share Feedback" and "Report an Issue" buttons — the issue button links to `repoUrl/issues` automatically.

When either field is empty (`''`), the corresponding CTA is hidden. No setup needed by default.

### Role Chip Colors

Customize the background color of role chips per role name. Text color is automatically calculated (black or white) based on contrast.
...
export const siteConfig = {
  ...
  subscribeUrl: 'https://buttondown.com/acme-docs',
  feedbackUrl: 'https://forms.gle/acme-feedback',
  blog: {
    layout: 'minimal' as 'modern' | 'minimal',
  },
  releaseNotes: {
