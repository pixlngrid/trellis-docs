...
The widget then renders automatically on every documentation topic. No per-page MDX changes are needed.
...
**Disadvantages:**
- All or nothing — every doc page gets the widget, including pages where feedback may not be relevant (e.g. changelogs, index pages).
- To exclude specific pages, you would need to add conditional logic in the layout (e.g. checking `doc.meta` for an opt-out flag).
...
If you need to use it outside of MDX (e.g. in a custom page or layout), import it explicitly:
...
- Good for sites where only a subset of pages benefit from feedback (e.g. tutorials, guides) while others don't (e.g. reference tables, legal pages).

**Disadvantages:**
- Easy to forget — authors must remember to add `<Feedback />` to each page that needs it.
- Inconsistent — different pages may or may not have the widget depending on who authored them.
...
1. The reader clicks thumbs up or thumbs down.
2. A modal appears with contextual options (e.g. "Easy to follow" for likes, "Hard to understand" for dislikes).
...
| Feedback modal | Displays contextual checkbox options and an optional text field after the reader clicks a thumb button. |
...
If your backend lives at a different URL, update this function. For multi-environment deployments (e.g. dev, staging, production), you can extend the logic based on `window.location.hostname`:
...
All servers run on port `3002` by default, matching the `getBaseUrl()` development configuration. Start your docs site with `npm run dev` and both the widget and dashboard will connect automatically.
...
|---|---|---|
| `MONGODB_URI` | Yes | MongoDB connection string (e.g. `mongodb://localhost:27017` or your Atlas URI). |
...
| Variable | Required | Description |
|---|---|---|
| `COSMOS_ENDPOINT` | Yes | Cosmos DB account URI (e.g. `https://your-account.documents.azure.com:443/`). |
...
Trellis includes a built-in dashboard for viewing and analyzing collected feedback. It is intended for doc admins and content owners — not site readers — and is accessed directly at `/feedback-dashboard/`. It is not linked in the site navigation.
...
| Feedback by page | Sortable table of every page that received feedback, with inline satisfaction bars. Click column headers to sort. |
...
- **Modal not opening:** Verify that the `Feedback` component is rendered client-side. It uses the `'use client'` directive, so it should work in any MDX page or layout, but will not work if imported from a server component at module scope.
...
- **CORS errors in development:** If your backend runs on a different port (e.g. `3002`) than the dev server, ensure the backend sets appropriate `Access-Control-Allow-Origin` headers.
- **API endpoint returning errors:** Verify your backend logs for exceptions during feedback processing. Confirm the database connection is healthy.
