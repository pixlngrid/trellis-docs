...
| **Image lightbox** | Click any image to open a full-screen modal. Supports width sizing via `![alt](src "50%")`. Add `"nozoom"` to disable zoom or `"noborder"` to drop the default frame on logos/icons. | Zero markup — every image gets lightbox behavior by default, with per-image opt-outs. |
...
| **Relationship tables** | DITA-style reltable (`content/reltable.yml`) drives automatic "Related topics" blocks across concept, task, reference, troubleshooting, and overview pages. | Maintain bidirectional related links in one place instead of editing every page — familiar to anyone with a structured-authoring background. |
...
| Feature | What you can do | Problem it solves |
|---------|----------------|-------------------|
| **Google Analytics** | Add your GA4 measurement ID in `config/site.ts` and analytics loads automatically with `afterInteractive` strategy. | Zero-code analytics setup. Toggle on/off with a config flag — no script tags to manage. |
| **Open Graph and Twitter Cards** | Every page gets OG and Twitter meta tags (title, description, URL, image) generated from frontmatter and site config. | Social shares and link unfurlers show rich previews without any per-page markup. |
| **JSON-LD structured data** | WebSite schema on the root layout, Article schema on every doc page with headline, description, URL, and dateModified. | Search engines understand your content structure for rich results — no manual schema authoring. |
| **Sitemap and robots.txt** | Build-time generation of `sitemap.xml` (all docs, blog, release notes) and `robots.txt`. Respects feature flags. | Search engines discover every page. No manual XML editing or third-party sitemap plugins. |
| **Canonical URLs** | Every page gets a `<link rel="canonical">` tag. i18n pages get `hreflang` alternates. | Prevents duplicate content penalties across locales and URL variations. |
| **Keywords meta tag** | Frontmatter `keywords` array renders as `<meta name="keywords">` and powers search ranking. | One field serves both internal search and external SEO — no duplication. |
...
| Feature | What you can do | Problem it solves |
|---------|----------------|-------------------|
| **CLI scaffolder** | `npx @patriciamcphee/create-trellis-docs my-docs` — interactive prompts, automatic platform detection, dependency installation. | Go from zero to running docs site in under a minute. |
| **CLI upgrade** | `npx @patriciamcphee/create-trellis-docs@latest upgrade` — updates framework files using an allow-list. Your content, config, landing page, and design tokens are never touched. | Upgrade without diffing templates manually. `--dry-run` to preview changes first. |
...
| **Docusaurus migration** | Automated migration script that copies content, converts sidebar, copies custom components, rewrites imports, and registers them in the MDX component map. | Switch frameworks without a multi-week rewrite — custom components are ported automatically. |
