---
title: Getting Started
description: Get up and running with the Trellis docs framework.
keywords: [getting-started, setup, introduction]
last_update:
  date: 02/20/2026
  author: Trellis
---

Welcome to **{vars.productName}** — an opinionated docs framework built on {vars.frameworkName} with custom theme enhancements, smart search, and a design token system.

## Prerequisites

Before getting started, make sure you have the following installed:

- **Node.js 18+** — [download here](https://nodejs.org/)
- **npm**, **yarn**, or **pnpm** — npm comes bundled with Node.js
- **Git** — [download here](https://git-scm.com/)

You can verify your setup by running:

```bash
node --version   # Should print v18.x or higher
npm --version    # Should print 9.x or higher
git --version
```

## What's included

| Category | Highlights | Learn More |
|----------|-----------|------------|
| **Theme** | Last-updated at top, heading copy-to-clipboard, pill-style tabs, custom admonition icons | [Theme Overview](/guides/style-and-layout/) |
| **Design Tokens** | JSON-to-CSS pipeline for centralized brand management | [Design Tokens](/guides/style-and-layout/) |
| **Plugins** | Smart search, FAQ indexer, redirects, image lightbox, Mermaid pan/zoom | [Plugins Overview](/guides/search/) |
| **Components** | Glossary, feedback widget, flipping cards, search UI | [Components](/guides/components/) |

New to {vars.productName}? Read [What is {vars.productName}?](/introduction/) for the full picture, or see the [feature comparison](/introduction/) with vanilla Next.js.

## Quick start

Scaffold a new project with a single command:

```bash
npx create-trellis-docs my-docs
cd my-docs
npm run dev
```

The scaffolder prompts you for a site title, tagline, URL, and optional GitHub repository URL.

:::tip
The dev server runs on port 3000 by default. Visit `http://localhost:3000` to see your docs.
:::

### Platform note

Tailwind CSS v4 relies on platform-specific native binaries (via `lightningcss`). The scaffolder detects your OS automatically, but if you run `npm install` manually and your `.npmrc` overrides the `os` setting, npm might skip the correct binaries. If you see an error like `Cannot find module '../lightningcss.<platform>.node'`, reinstall with your actual platform:

```bash
# macOS
npm install --os=darwin

# Windows
npm install --os=win32

# Linux
npm install --os=linux
```

## Project structure

```
my-docs/
├── app/
│   ├── (docs)/            # Docs layout (navbar, sidebar, footer)
│   ├── blog/              # Blog pages
│   ├── release-notes/     # Release notes pages
│   ├── layout.tsx         # Root layout (providers, fonts)
│   ├── page.tsx           # Landing page
│   ├── globals.css        # Global CSS + dark/light mode tokens
│   └── tokens.css         # Generated design token CSS
├── components/
│   ├── blog/              # Blog sidebar
│   ├── brand/             # Logo components (replace with your own)
│   ├── custom/            # Reusable components (glossary, feedback, etc.)
│   └── docs/              # Layout & MDX components
├── config/
│   ├── navigation.ts      # Navbar and footer links
│   ├── sidebar.ts         # Sidebar navigation structure
│   ├── site.ts            # Site title, logo, search, i18n, versioning
│   └── variables.ts       # Reusable content variables
├── content/
│   ├── docs/              # Documentation content (MDX files)
│   ├── blog/              # Blog posts
│   └── release-notes/     # Version release notes
├── data/                  # Static data files (glossary, etc.)
├── lib/                   # Content loading, routing, utilities
├── public/img/            # Static assets (images, logos, favicon)
├── scripts/               # Build scripts (tokens, search, FAQ, versioning)
├── design-tokens.json     # Design token definitions
├── redirects.json         # URL redirect mappings
└── package.json
```

## Next steps

- [What is {vars.productName}?](/introduction/) — understand the framework and what it provides
- [{vars.productName} vs {vars.frameworkName}](/introduction/) — feature comparison
- [Site Configuration](/guides/site-configuration/) — customize site identity, navigation, and sidebar
- [Content Authoring](/guides/docs/) — create pages, blog posts, and release notes
- [Writing Docs](/guides/markdown/) — markdown features (code blocks, admonitions, tabs)
- [Design Tokens](/guides/style-and-layout/) — customize colors and branding
- [Deployment](/guides/deployment/) — build and deploy your site