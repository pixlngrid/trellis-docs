...
export function DocCard({ title, description, href }: DocCardProps) {
  return (
    ...
    >
      <h3 className="m-0 text-base font-semibold text-[var(--foreground)] group-hover:text-[var(--primary)]">
        {title}
      </h3>
      {description && (
        <p className="m-0 mt-1.5 text-sm text-[var(--muted-foreground)] line-clamp-2">
          {description}
        </p>
      )}
    </Link>
