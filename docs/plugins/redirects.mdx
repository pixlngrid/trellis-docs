...
3. Each HTML file contains a `<meta http-equiv="refresh">` tag and a JavaScript fallback
4. Both `path.html` and `path/index.html` are created to handle trailing slash variations
...
The plugin respects the `trailingSlash` setting in `docusaurus.config.js`:
