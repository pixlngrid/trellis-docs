...
2. **Index generation** — a `searchIndex.json` file is written to the `static/` directory
...
The search interface is provided by the `CustomSearch` component in `src/components/CustomSearch/`. It renders as a search icon in the navbar that opens a modal with:
