---
title: FAQ Index
description: How the FAQ index plugin automatically generates a searchable table of contents from FAQ pages.
keywords: [faq, index, plugin, questions, table of contents]
last_update:
  date: 02/20/2026
  author: Trellis
---

The FAQ index plugin scans your FAQ directory for `###` headings, treats each one as a question, and exposes the data so the `FaqTableOfContents` component can render a searchable, filterable question list.

## How It Works

1. **Scans** all `.mdx` and `.md` files in the configured FAQ directory (excluding `index.mdx`)
2. **Extracts** `###` headings from each file as questions
3. **Generates** anchor links from the heading text
4. **Exposes** the data via Docusaurus `setGlobalData` so any component can consume it

## Configuration

```js title="docusaurus.config.js"
['./packages/faq-index-plugin', {
  faqDir: 'docs/faq',       // Directory containing FAQ files
  basePermalink: '/faq',    // Base URL for FAQ pages
}]
```

## Writing FAQ Pages

Each FAQ page is a standard MDX file in the `docs/faq/` directory. Use `###` headings for questions:

```mdx title="docs/faq/general.mdx"
---
title: General
description: General questions about the framework.
---

### What is Trellis?

Trellis is an opinionated documentation framework built on Docusaurus...

### How do I customize the colors?

Edit `design-tokens.json` at the project root...
```

The plugin extracts each `###` heading as a question and generates an anchor from the text:
- `### What is Trellis?` → anchor: `what-is-trellis`
- `### How do I customize the colors?` → anchor: `how-do-i-customize-the-colors`

## FaqTableOfContents Component

The companion component renders the indexed questions as a searchable list. Import and use it on your FAQ index page:

```mdx title="docs/faq/index.mdx"
---
title: Frequently Asked Questions
slug: /faq
hide_table_of_contents: true
---

import FaqTableOfContents from '@site/src/components/FaqTableOfContents';

Common questions about the framework.

<FaqTableOfContents />
```

The component:
- Groups questions by FAQ page (topic)
- Provides a text filter for searching across all questions
- Links each question directly to its anchor on the source page

## Data Structure

The plugin generates the following data structure, accessible via `useGlobalData`:

```js
// useGlobalData()['docusaurus-plugin-faq-index'].default.topics
[
  {
    slug: 'general',
    title: 'General',
    description: 'General questions about the framework.',
    permalink: '/faq/general',
    questions: [
      { text: 'What is Trellis?', anchor: 'what-is-trellis' },
      { text: 'How do I customize the colors?', anchor: 'how-do-i-customize-the-colors' },
    ],
  },
]
```
